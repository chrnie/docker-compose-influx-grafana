...
influx bucket create -n telegraf -r 168h
TELEGRAF_BUCKET=$(influx bucket list |grep -P "\stelegraf\s" | grep -Po "^\S*")

echo "create READ and WRITE token"
influx auth create --read-buckets -d "READ BUCKETS"
influx auth create --write-bucket $TELEGRAF_BUCKET -d "WRITE telegraf"
